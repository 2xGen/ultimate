...
import { BG_CANONICAL_SLUG } from "../../../lib/bg-config";

const COCONUT_REF = "ref=arubabuddies";
const OG_DESCRIPTION =
  "Rent beach chairs, umbrellas, snorkel gear, and more in Aruba—delivered with Coconut Rentals.";
const OG_IMAGE_URL =
...
function buildCoconutUrl(originalSearch: string): string {
  const queryWithoutQuestion = originalSearch.startsWith("?")
    ? originalSearch.slice(1)
    : originalSearch;
  const mergedQuery = queryWithoutQuestion
    ? `${queryWithoutQuestion}&${COCONUT_REF}`
    : COCONUT_REF;

  return `https://coconutrentals.com/?${mergedQuery}`;
}

async function getCurrentBgUrl(slug: string, originalSearch: string): Promise<string> {
  const h = await headers();
  const host = h.get("x-forwarded-host") || h.get("host") || "ultimate-travel-tips.com";
  const proto = h.get("x-forwarded-proto") || "https";
  return `${proto}://${host}/bg/${encodeURIComponent(slug)}${originalSearch}`;
}

const PAGE_TITLE = "Beach Gear Rentals In Aruba";
    ...
    : `<main class="hero">
        <div class="card">
          <div class="top-row">
            ...
            <span class="badge">Coconut Rentals</span>
          </div>
          <h1>${safeTitle}</h1>
          <p class="line">Taking you to beach gear rentals in Aruba...</p>
          <p class="line subtle" id="status">Loading rental options...</p>
          <div class="activity">
            <span class="spinner" aria-hidden="true"></span>
            <span>Opening Coconut Rentals in 1 second.</span>
          </div>
          <ul class="steps" aria-label="Redirect progress">
            <li class="step active" id="step-1">Finding beach gear</li>
            <li class="step" id="step-2">Applying partner link</li>
            <li class="step" id="step-3">Opening Coconut Rentals</li>
          </ul>
      ...
      <script>
        (function () {
          ...
          var timer = setInterval(function () {
            ...
            if (status && ratio < 0.4) status.textContent = "Loading rental options...";
            if (status && ratio >= 0.4 && ratio < 0.85) status.textContent = "Almost there...";
            if (status && ratio >= 0.85) status.textContent = "Opening Coconut Rentals...";
...
export async function GET(
  ...
  const destination = buildCoconutUrl(originalSearch);
  const currentUrl = await getCurrentBgUrl(slug, originalSearch);
  const h = await headers();
