...
import {
  BQ_GENERIC_SLUG,
  BQ_LEGACY_SHORT_TO_PLACE_SLUG,
} from "../../../lib/bq-config";

const BABYQUIP_AFFILIATE = "a=66993aa";
const OG_DESCRIPTION =
  "Rent baby gear delivered to your door—strollers, cribs, car seats, and more for stress-free family travel.";
const OG_IMAGE_URL =
  "https://iemgpccgdlwpsrsjuumo.supabase.co/storage/v1/object/public/tour-fotos/ultimate-baby-equipment-og.jpg";
...
function buildBabyQuipUrl(originalSearch: string): string {
  const queryWithoutQuestion = originalSearch.startsWith("?")
    ? originalSearch.slice(1)
    : originalSearch;
  const mergedQuery = queryWithoutQuestion
    ? `${queryWithoutQuestion}&${BABYQUIP_AFFILIATE}`
    : BABYQUIP_AFFILIATE;

  return `https://www.babyquip.com/?${mergedQuery}`;
}

async function getCurrentBqUrl(slug: string, originalSearch: string): Promise<string> {
  const h = await headers();
  const host = h.get("x-forwarded-host") || h.get("host") || "ultimate-travel-tips.com";
  const proto = h.get("x-forwarded-proto") || "https";
  return `${proto}://${host}/bq/${encodeURIComponent(slug)}${originalSearch}`;
}

function titleForSlug(slug: string): string {
  if (slug === BQ_GENERIC_SLUG) {
    return "Best Baby Gear Options";
  }
  const match = /^baby-gear-in-(.+)$/.exec(slug);
  if (match) {
    const place = decodeURIComponent(match[1])
      .replaceAll("-", " ")
      .trim();
    return titleCaseWords(place ? `Baby Gear In ${place}` : "Baby Gear");
  }
  return "Baby Gear";
}
    ...
    : `<main class="hero">
        <div class="card">
          <div class="top-row">
            ...
            <span class="badge">BabyQuip</span>
          </div>
          <h1>${safeTitle}</h1>
          <p class="line">Finding the best baby gear for your trip...</p>
          <p class="line subtle" id="status">Loading equipment options...</p>
          <div class="activity">
            <span class="spinner" aria-hidden="true"></span>
            <span>We are opening BabyQuip for you in 1 second.</span>
          </div>
          <ul class="steps" aria-label="Redirect progress">
            <li class="step active" id="step-1">Loading baby gear options</li>
            <li class="step" id="step-2">Finding the best fit</li>
            <li class="step" id="step-3">Opening BabyQuip</li>
          </ul>
      ...
      <script>
        (function () {
          ...
          var timer = setInterval(function () {
            ...
            if (status && ratio < 0.4) status.textContent = "Loading equipment options...";
            if (status && ratio >= 0.4 && ratio < 0.85) status.textContent = "Finding the best gear...";
            if (status && ratio >= 0.85) status.textContent = "Opening BabyQuip...";
...
export async function GET(
  ...
  const legacyTarget = BQ_LEGACY_SHORT_TO_PLACE_SLUG[slug];
  if (legacyTarget) {
    const dest = new URL(request.url);
    dest.pathname = `/bq/baby-gear-in-${legacyTarget}`;
    dest.search = originalSearch;
    return NextResponse.redirect(dest, 308);
  }

  const isGeneric = slug === BQ_GENERIC_SLUG;
  const placeMatch = /^baby-gear-in-(.+)$/.exec(slug);
  ...
  const destination = buildBabyQuipUrl(originalSearch);
  const title = titleForSlug(slug);
  const currentUrl = await getCurrentBqUrl(slug, originalSearch);
  const h = await headers();
