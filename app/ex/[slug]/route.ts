...
import { exBrandLabel, getExEntry, mergeExQuery, type ExEntry } from "../../../lib/ex-config";

function ogDescriptionForEntry(entry: ExEntry): string {
  return `Hotels and vacation rentals — ${entry.label}. Opens with our partner link on ${exBrandLabel(entry.slug)}.`;
}
...
async function getCurrentExUrl(slug: string, originalSearch: string): Promise<string> {
  const h = await headers();
  const host = h.get("x-forwarded-host") || h.get("host") || "ultimate-travel-tips.com";
  const proto = h.get("x-forwarded-proto") || "https";
  return `${proto}://${host}/ex/${encodeURIComponent(slug)}${originalSearch}`;
}
  ...
  const bodyForHumans = isBot
    ...
    : `<main class="hero">
        <div class="card">
          ...
          <h1>${safeTitle}</h1>
          <p class="line">Taking you to hotels and vacation rentals on ${escapeHtml(brand)}...</p>
          <p class="line subtle" id="status">Loading offers...</p>
          <div class="activity">
            <span class="spinner" aria-hidden="true"></span>
            <span>Redirecting in 1 second with our partner link.</span>
          </div>
          <ul class="steps" aria-label="Redirect progress">
            <li class="step active" id="step-1">Opening partner link</li>
            <li class="step" id="step-2">Applying affiliate parameters</li>
      ...
      <script>
        (function () {
          ...
          var timer = setInterval(function () {
            ...
            if (status && ratio < 0.4) status.textContent = "Loading offers...";
            if (status && ratio >= 0.4 && ratio < 0.85) status.textContent = "Almost there...";
            if (status && ratio >= 0.85) status.textContent = "Opening " + brand + "...";
...
export async function GET(
  ...
  const destination = mergeExQuery(entry.destinationUrl, originalSearch);
  const title = entry.label;
  const ogDescription = ogDescriptionForEntry(entry);
  const currentUrl = await getCurrentExUrl(slug, originalSearch);
  const brand = exBrandLabel(entry.slug);
  const badge = brand === "Vrbo" ? "Vrbo" : "Expedia";
